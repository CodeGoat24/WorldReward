...
def angle_bucket(theta):
    """Bucket an angle into [-1, 0, +1] classes."""
    out = torch.zeros_like(theta, dtype=torch.int)
    out[(theta >= 0) & (theta < 30)] = 1  # positive
    out[(theta >= 150) & (theta <= 180)] = -1  # negative
    # 30-60 or 120-150 -> diagonal, marked 2 or -2
    out[(theta >= 30) & (theta < 60)] = 2  # positive diagonal
    out[(theta >= 120) & (theta < 150)] = -2  # negative diagonal
    return out


def angle_to_onehot(x):
    """
    x: (B,2) angles (theta_x, theta_y) in [0,180]
    return: (B,4) one-hot vector [right, left, backward, forward]
    """
    B = x.size(0)
    ...
    # theta_x controls forward/backward
    one_hot[bucket_x == 1, 3] = 1  # forward
    one_hot[bucket_x == -1, 2] = 1  # backward
    one_hot[bucket_x == 2, 3] = 1  # forward diagonal -> forward
    one_hot[bucket_x == -2, 2] = 1  # backward diagonal -> backward

    # theta_y controls left/right
    one_hot[bucket_z == 1, 0] = 1  # right
    one_hot[bucket_z == -1, 1] = 1  # left
    one_hot[bucket_z == 2, 0] = 1  # right diagonal -> right
    one_hot[bucket_z == -2, 1] = 1  # left diagonal -> left
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                # Zero-pad the latent temporal dim to self.window_frames
                ...
                # Compute the discrete action from here on
                ...
                for i in range(1, relative_c2w.shape[0]):
                    ...
                    # Thresholds: movement norm and rotation degrees
                    if move_norms > move_norm_valid:  # counts as movement
                        # forward/backward from the z axis
                    ...
                    # Rotation: the degree threshold may need tuning per data source
...
def hunyuan_latent_collate_function(batch):
    ...
    latent_path = [b.get("latent_path", "") for b in batch]
