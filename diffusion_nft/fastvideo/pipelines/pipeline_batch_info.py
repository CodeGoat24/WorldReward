...
from fastvideo.layers.attention import AttentionMetadata
...
class TrainingBatch:
    ...
    vlm_action_reward_mean: float = 0.0
    vlm_vq_reward_mean: float = 0.0
    ate_rmse_reward_mean: float = 0.0
    rpe_trans_reward_mean: float = 0.0
    rpe_rot_reward_mean: float = 0.0
    aesthetic_reward_mean: float = 0.0
    ...
    # Generic per-output reward-mean bag, populated by RewardDispatcher.
    # Keys are output names from `dispatcher.all_outputs` (e.g. "action",
    # "vlm_action", "ate_rmse"); values are scalar floats averaged across
    # the full world group. Wandb logging iterates this instead of reading
    # the hardcoded per-reward fields above, so adding a new reward only
    # requires registering the computer class.
    reward_means: dict[str, float] = field(default_factory=dict)
