...
class ComposedPipelineBase(ABC):
    ...
    This class provides the framework for creating pipelines by composing multiple stages together.
    Each stage is responsible for a specific part of the diffusion process, and the pipeline
    orchestrates the execution of these stages.
    """
    ...
    def __init__(
        ...
        After __init__, the pipeline should be ready to use. The pipeline should be stateless and
        not hold any batch state.
        """
        ...
        initialize_gpu_parallel_group(fastvideo_args.gpu_para)

        # Load modules directly in initialization
        logger.info("Loading pipeline modules...")
        # print(fastvideo_args, loaded_modules)
        # quit()

        if "Hunyuan" in fastvideo_args.cls_name:
            self.modules = self.load_hunyuan_modules(fastvideo_args)
        else:
    ...
    def required_config_modules(self) -> list[str]:
        """List of modules that are required by the pipeline. The names should match the diffusers
        directory and model_index.json file. These modules will be loaded using the
        PipelineComponentLoader and made available in the modules dictionary. Access these modules
        using the get_module method.
    ...
    def load_hunyuan_modules(
        ...
        component_model_path = fastvideo_args.load_from_dir
        fastvideo_args.module_name = module_name
    ...
    def load_modules(
        ...
        If provided, loaded_modules will be used instead of loading from config/pretrained weights.
        """
        # Resolve the paths of the models to load
        ...
        for module_name in self.required_config_modules:
            if (
                ...
                logger.warning(
                    "model_index.json does not contain a %s module, but found {%s: %s}.",
                    module_name,
                    module_name,
                    extra_module_value,
                )
                if extra_module_value in model_index:
                    ...
                    raise ValueError(
                        f'''Required module key: {module_name} value: {model_index.get(module_name)}
                         was not found in loaded modules {model_index.keys()}'''
                    )
        ...
        # Load the required models here, e.g. the DiT
        modules = {}
        # print(model_index, self.required_config_modules)
        for module_name, (
            ...
            component_model_path = os.path.join(
                self.model_path, load_module_name
            )
            # Distinguish between different transformer DiTs
            fastvideo_args.module_name = module_name
        ...
        for module_name in required_modules:
            if module_name not in modules or modules[module_name] is None:
                raise ValueError(
                    f'''Required module key: {module_name} value: {modules.get(module_name)} 
                    was not found in loaded modules {modules.keys()}'''
                )
    ...
    def forward(
        ...
        # Execute each stage
        logger.info(
            "Running pipeline stages: %s", self._stage_name_mapping.keys()
        )
