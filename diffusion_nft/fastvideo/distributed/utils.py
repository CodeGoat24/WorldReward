...
class StatelessProcessGroup:
    """A dataclass to hold a metadata store, and the rank, world_size of the group.

    Only use it to communicate metadata between processes. For data-plane communication, create
    NCCL-related objects.
    """
    ...
    def broadcast_obj(self, obj: Any | None, src: int) -> Any:
        ...
        It does not clean up after all ranks have received the object. Use it for limited times,
    ...
    def create(
        ...
        """A replacement for `torch.distributed.init_process_group` that does not pollute the global
        state.
