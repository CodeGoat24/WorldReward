...
class GroupCoordinator:
    ...
    PyTorch ProcessGroup is bound to one specific communication backend,     e.g. NCCL, Gloo, MPI,
    etc. GroupCoordinator takes charge of all the communication operations among     the processes
    in the group. It manages both CPU and device     communication.
    """
    ...
    def __init__(
        ...
        _subgroup_timeout = timedelta(minutes=30)
        for ranks in group_ranks:
            device_group = torch.distributed.new_group(
                ranks, backend=torch_distributed_backend, timeout=_subgroup_timeout
            )
            # a group with `gloo` backend, to allow direct coordination between
            # processes through the CPU.
            cpu_group = torch.distributed.new_group(
                ranks, backend="gloo", timeout=_subgroup_timeout
            )
    ...
    def all_to_all_4D(
        ...
        if self.world_size == 1:
            return input_
        return self.device_communicator.all_to_all_4D(
...
_GPU: GroupCoordinator | None = None


def get_gpu_group() -> GroupCoordinator:
    assert _GPU is not None, "gpu parallel group is not initialized"
    return _GPU
...
def init_distributed_environment(
    ...
    # Extend collective timeout to 30 minutes. Default 10 min triggers
    # watchdog during slow VLM reward phase (wake-up + pairwise inference).
    from datetime import timedelta
    ...
    if not torch.distributed.is_initialized():
        ...
        # MPS does not support device indices, so device_id is dropped there.
        # FASTVIDEO_NCCL_NO_COMM_SPLIT=1 drops it too: with device_id set,
        # DeviceMesh builds sub-meshes via ncclCommSplit, which segfaults
        # non-deterministically on NCCL 2.21.5 during HSDP sub-mesh creation.
        _no_comm_split = os.environ.get("FASTVIDEO_NCCL_NO_COMM_SPLIT", "0") == "1"
        if current_platform.is_mps() or _no_comm_split:
            if _no_comm_split:
                logger.info(
                    "FASTVIDEO_NCCL_NO_COMM_SPLIT=1: omitting device_id so "
                    "DeviceMesh uses new_group() instead of ncclCommSplit"
                )
    ...
    # Belt-and-suspenders: override the default PG's per-collective timeout
    # at the C++ backend level. init_process_group(timeout=...) sets a
    # Python timedelta, but the NCCL C++ backend may still use
    # _DEFAULT_PG_NCCL_TIMEOUT (10 min) for its watchdog unless we call
    # _set_default_timeout on the backend directly.
    try:
        default_pg = torch.distributed.distributed_c10d._get_default_group()
        if hasattr(default_pg, "_get_backend"):
            backend_pg = default_pg._get_backend(torch.device("cuda"))
            if hasattr(backend_pg, "_set_default_timeout"):
                backend_pg._set_default_timeout(nccl_timeout)
    except Exception as _e:
        logger.warning("Failed to set NCCL backend timeout: %s", _e)
    # set the local rank
...
def initialize_gpu_parallel_group(
    gpu_parallel_size: int = 1,
    backend: str | None = None,
) -> None:
    global _GPU
    assert _GPU is None, "gpu parallel group is not initialized"
    world_size: int = get_world_size()
    num_tensor_model_parallel_groups: int = world_size // gpu_parallel_size
    group_ranks = []
    for i in range(num_tensor_model_parallel_groups):
        ranks = list(range(i * gpu_parallel_size, (i + 1) * gpu_parallel_size))
        group_ranks.append(ranks)

    _GPU = init_model_parallel_group(
        group_ranks,
...
def initialize_model_parallel(
    ...
    num_sequence_model_parallel_groups: int = (
        world_size // sequence_model_parallel_size
    )
    global _SP
    ...
    for i in range(num_sequence_model_parallel_groups):
        ...
        ranks = list(
            range(
                i * sequence_model_parallel_size,
...
def get_gpu_world_size() -> int:
    """Return world size for the gpu parallel group."""
    return get_gpu_group().world_size


def get_gpu_rank() -> int:
    """Return my rank for the gpu parallel group."""
    return get_gpu_group().rank_in_group
...
def is_the_same_node_as(
    ...
    """This is a collective operation that returns if each rank is in the same node as the source
    rank.

    It tests if processes are attached to the same memory system (shared access to shared memory).
    """
