...
def zeropower_via_newtonschulz5(G, steps=5):
    ...
    something like US'V^T where S' is diagonal with S_{ii}' ~ Uniform(0.5, 1.5), which turns out not
...
class Muon(torch.optim.Optimizer):
    ...
    def materialize_state(self) -> None:
        """Eagerly allocate the state buffers that step() creates lazily.

        __init__ only records the non-tensor ``use_muon`` flag; the actual
        buffers (``momentum_buffer`` for Muon params, ``step``/``moment1``/
        ``moment2`` for the AdamW params) are allocated on the first step()
        call. Resuming loads the optimizer state BEFORE any step has run, so
        without this there are no destination tensors for the saved buffers and
        the distributed-checkpoint load has nowhere to put them.

        Shapes, dtypes, devices and (under FSDP) DTensor sharding are taken
        from the parameter, which matches its grad exactly, so what this
        creates is identical to what step() would create — only earlier.
        Idempotent: existing buffers are left untouched.
        """
        for group in self.param_groups:
            for p in group["params"]:
                state = self.state[p]
                if state.get("use_muon"):
