...
from fastvideo.utils.checkpointing_utils import (
    EMAWrapper,
    ModelWrapper,
    OptimizerWrapper,
    RandomStateWrapper,
...
def remove_previous_state(output_dir, rank, total_limit: int = 0):
    """Trim old checkpoints.

    Two things happen:
      1. Strip the heavy `distributed_checkpoint/` subdir from every old
         checkpoint (keeps the consolidated transformer/*.safetensors for
         inference but gives up full resume). Always done — this is the
         existing behavior.
      2. If `total_limit > 0`, delete entire checkpoint directories
         (including the consolidated weights) so the on-disk count stays
         at most `total_limit` after this save. Keeps the newest ones by
         step number.
    """
    checkpoint_dir_list = os.listdir(output_dir)
    # Drop distributed_checkpoint/ from old ckpts. The newly created
    # one for the current save does not exist yet at this point, so all
    # dcp dirs here belong to strictly older checkpoints.
    ...
    if rank != 0 or total_limit <= 0:
        return

    # Enforce total_limit. We haven't written the NEW checkpoint yet;
    # after it lands, on-disk count will be existing + 1. So drop the
    # oldest until we leave room for it: len(existing) <= total_limit - 1.
    ckpts = []
    ...
    ckpts.sort(key=lambda x: x[0])  # oldest first

    max_keep_after = max(0, total_limit - 1)
    while len(ckpts) > max_keep_after:
        step, path = ckpts.pop(0)
        try:
            shutil.rmtree(path)
            logger.info(
                "Removed old checkpoint %s to respect checkpoints_total_limit=%d",
                path, total_limit,
            )
...
def save_checkpoint(
    ...
    Args:
        checkpoints_total_limit: If > 0, cap the number of on-disk
            checkpoint directories at this value (keeps newest by step).
            0 = keep all (legacy behavior).
        config_path: If given, the training YAML at this path is copied into
            the checkpoint dir as ``training_config.yaml`` (rank 0 only) so
            the run is reproducible from the checkpoint alone.
    """
    if os.path.exists(output_dir):
        remove_previous_state(output_dir, rank, total_limit=checkpoints_total_limit)
    # Barrier so remote ranks don't race ahead and mkdir while rank 0 is
    # still rmtree-ing the previous checkpoint directory.
    ...
    if ema_generator is not None:
        states["ema"] = EMAWrapper(ema_generator)
    dcp_dir = os.path.join(save_dir, "distributed_checkpoint")
    logger.info(
    ...
    # Every rank creates the dir (mkdir is idempotent with exist_ok=True):
    # cross-node FS visibility of rank 0's mkdir can lag, and a rank racing
    # ahead may otherwise create its .distcp entry as a regular file and fail.
    ...
    if rank == 0:
        ...
        config_path_json = os.path.join(transformer_save_dir, "config.json")
        # save dict as json
        with open(config_path_json, "w") as f:
            json.dump(config_dict, f, indent=4)
        logger.info("--> checkpoint saved at step %s to %s", step, weight_path)

        # Archive the training YAML alongside the checkpoint for reproducibility.
        if config_path and os.path.isfile(config_path):
            try:
                shutil.copyfile(
                    config_path, os.path.join(save_dir, "training_config.yaml")
                )
                logger.info(
                    "--> training config archived to %s",
                    os.path.join(save_dir, "training_config.yaml"),
                )
            except Exception as e:
                logger.warning("failed to archive training config %s: %s", config_path, e)
...
def load_checkpoint(
    ...
    if not os.path.exists(checkpoint_path):
        # Must NOT degrade to a from-scratch run: returning 0 here would leave
        # the model at its base weights while init_steps=0 also rewinds the
        # progressive_min2max curriculum to chunk 1, producing a run that looks
        # like a resume in the logs but is silently a fresh one.
        raise FileNotFoundError(
            f"resume_from_checkpoint points at {checkpoint_path!r}, which does "
            "not exist. Refusing to silently start from scratch."
        )
    ...
    if not os.path.exists(dcp_dir):
        # remove_previous_state() strips distributed_checkpoint/ from every
        # older checkpoint on each save (25 GB apiece), so only the NEWEST
        # checkpoint is resumable. An older one still has transformer/ and is
        # fine to evaluate, just not to resume.
        raise FileNotFoundError(
            f"{checkpoint_path} has no distributed_checkpoint/ — it cannot be "
            "resumed from. Only the newest checkpoint keeps one; older ones are "
            "stripped to save space. Refusing to silently start from scratch."
        )
...
class EMA_FSDP_schedule:
    ...
    def __init__(
        ...
        # name -> (device_mesh, placements, global_shape, global_stride) for
        # every sharded param, filled by _init_shadow. See _record_dtensor_spec.
        self._dtensor_spec: dict[str, tuple] = {}
    ...
    def _init_shadow(self, module):
        ...
        for name, p in module.named_parameters():
            ...
            self._record_dtensor_spec(name, p)

    def _record_dtensor_spec(self, name: str, p: torch.Tensor) -> None:
        """Remember how ``p`` is sharded so state_dict() can hand DCP a DTensor.

        The shadows are plain CPU tensors holding this rank's *local shard*.
        dcp.save() has no way to know that: it treats a plain tensor as a
        replicated/global one, records the local shape as the global shape and
        writes a single chunk, which corrupts sharded entries and breaks
        resume for params with uneven shards (e.g. a 3-row embedding over 8
        shard ranks gives each rank a different local shape).

        Keeping the mesh/placements plus the GLOBAL shape and stride lets
        state_dict() rebuild an equivalent DTensor. shape/stride must be passed
        explicitly: from_local() otherwise infers global shape as
        local_shape * mesh_size, which is wrong for uneven shards
        ((1, 2048) would become (8, 2048)).
        """
        try:
    ...
    def _as_dtensor(self, name: str, v: torch.Tensor) -> torch.Tensor:
        """Wrap a local-shard shadow tensor back into a DTensor for DCP.

        Returns ``v`` unchanged when the param was never sharded (plain tensor
        params, or dist not initialized) -- those are genuinely replicated and
        DCP handles them correctly already.
        """
        spec = self._dtensor_spec.get(name)
        if spec is None:
            return v
        try:
            from torch.distributed.tensor import DTensor
            ...
            return v
        mesh, placements, shape, stride = spec
    ...
    @staticmethod
    def _as_local(v: torch.Tensor) -> torch.Tensor:
        """Inverse of _as_dtensor: DCP hands back DTensors, shadows want local."""
        try:
            from torch.distributed.tensor import DTensor
    ...
    def state_dict(self) -> dict[str, torch.Tensor]:
        # Serialize both shadows under namespaced keys so checkpoint resume
        # can restore the full EMA state. "ckpt.<param>" and "policy.<param>".
        # For mode="rank0_full", only rank 0 holds the full state.
        #
        # Each shadow is this rank's LOCAL shard, so it must be wrapped in a
        # DTensor before it reaches dcp.save() -- otherwise DCP records the
        # local shape as the global one. See _record_dtensor_spec.
        if self.mode == "rank0_full" and self.rank != 0:
            return {}
        sd: dict[str, torch.Tensor] = {}
        for k, v in self.ckpt_shadow.items():
            sd[f"ckpt.{k}"] = self._as_dtensor(k, v.clone())
        for k, v in self.policy_shadow.items():
            sd[f"policy.{k}"] = self._as_dtensor(k, v.clone())
        return sd

    def load_state_dict(self, sd: dict[str, torch.Tensor]):
        # Accept both new namespaced format and old ckpt-only format.
        # Values may arrive as DTensors (dcp.load fills the containers that
        # state_dict() returned), so unwrap them back to local CPU float32.
        if any(k.startswith("ckpt.") or k.startswith("policy.") for k in sd):
            ckpt: dict[str, torch.Tensor] = {}
            policy: dict[str, torch.Tensor] = {}
            for k, v in sd.items():
                if k.startswith("ckpt."):
                    ckpt[k[len("ckpt."):]] = self._as_local(v)
                elif k.startswith("policy."):
                    policy[k[len("policy."):]] = self._as_local(v)
            self.ckpt_shadow = ckpt
            if policy:
                self.policy_shadow = policy
            # If an old ckpt-only checkpoint is loaded, policy_shadow keeps
            # whatever _init_shadow populated from the current model — good
            # enough, since policy_shadow is rebuilt each step anyway.
        else:
            # Legacy format: only ckpt_shadow was saved.
            self.ckpt_shadow = {k: self._as_local(v) for k, v in sd.items()}
