...
def find_nccl_library() -> str:
    """We either use the library file specified by the `VLLM_NCCL_SO_PATH` environment variable, or
    we find the library file brought by PyTorch.
...
def current_stream() -> torch.cuda.Stream | None:
    """Replace `torch.cuda.current_stream()` with `fastvideo.utils.current_stream()`. it turns out
    that `torch.cuda.current_stream()` is quite expensive, as it will construct a new stream object
    at each call. here we patch `torch.cuda.set_stream` to keep track of the current stream
...
class FlexibleArgumentParser(argparse.ArgumentParser):
    ...
    def _pull_args_from_config(self, args: list[str]) -> list[str]:
        """Method to pull arguments specified in the config file into the command-line args
        variable.
        ...
        # CLI subcommands keep their leading positional tokens before config args.
        # Plain training/inference entrypoints usually start directly with options.
        if args[0] == "serve":
            if index == 1:
                raise ValueError(
    ...
    def _load_config_file(self, file_path: str) -> list[str]:
        ...
        def process_dict(prefix: str, d: dict[str, Any]):
            for key, value in d.items():
                if not prefix and key in semantic_groups and isinstance(value, dict):
                    process_dict("", value)
                    continue
                ...
                if (
                    ...
                    and full_key not in store_boolean_arguments
                ):
                    if value:
...
def import_pynvml():
    ...
    """
    import fastvideo.utils.pynvml as pynvml
...
def maybe_download_model(
    ...
    try:
        logger.info(
            "Downloading model snapshot from HF Hub for %s...",
            model_name_or_path,
        )
...
def run_method(
    ...
    If the method is string, it will be converted to a method using getattr. If the method is
    serialized bytes and will be deserialized using cloudpickle. If the method is a callable, it
...
def dict_to_3d_list(
    ...
    if mask_strategy is None:
        ...
        return [
            [[None for _ in range(h_max)] for _ in range(l_max)]
