...
class EMAWrapper(torch.distributed.checkpoint.stateful.Stateful):
    """Stateful wrapper so dcp.save/load can serialize the full EMA state
    (ckpt_shadow + policy_shadow). Must be paired with
    EMA_FSDP_schedule's namespaced state_dict/load_state_dict format
    ("ckpt.<param>" / "policy.<param>" keys).
    """

    def __init__(self, ema_generator) -> None:
        self.ema_generator = ema_generator

    def state_dict(self) -> dict[str, Any]:
        return {"ema": self.ema_generator.state_dict()}

    def load_state_dict(self, state_dict: dict[str, Any]) -> None:
        if "ema" in state_dict:
            self.ema_generator.load_state_dict(state_dict["ema"])
