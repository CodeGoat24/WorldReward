...
class TransformerLoader(ComponentLoader):
    ...
    def load(self, model_path: str, fastvideo_args: FastVideoArgs):
        ...
        # Resolve the model class; new model types must be registered here
        # cls_name = "ARWanTransformer3DModel"
        # print(fastvideo_args.module_name)
        if (
            fastvideo_args.module_name == "transformer"
        ):  # ar / ar_action models load their own weights
            cls_name = fastvideo_args.cls_name
            load_from_dir = fastvideo_args.load_from_dir
            ar_action_load_from_dir = fastvideo_args.ar_action_load_from_dir
        else:
            cls_name = "WanTransformer3DBidirectModel"
            load_from_dir = model_path
            ar_load_from_dir = None
        model_cls, _ = ModelRegistry.resolve_model_cls(cls_name)
        # print(model_cls, cls_name)
        # quit()
        # Find all safetensors files
        safetensors_list = glob.glob(
            os.path.join(str(model_path), "*.safetensors")
        ...
        logger.info(
            "Loading model from %s, default_dtype: %s", cls_name, default_dtype
        )
        assert fastvideo_args.hsdp_shard_dim is not None
        # print(model_path, model_cls)
        # quit()
