...
"""Minimal implementation of CLIPVisionModel intended to be only used within a vision language
model."""
...
from fastvideo.layers.attention import LocalAttention
...
class CLIPEncoder(nn.Module):
    """Transformer encoder consisting of `config.num_hidden_layers` self attention layers. Each
    layer is a [`CLIPEncoderLayer`].
...
class CLIPVisionModel(ImageEncoder):
    ...
    def forward(
        ...
        **kwargs,
        ...
        last_hidden_state = self.vision_model(
            pixel_values, feature_sample_layers
        )
