...
class DeepSeekClient(object):
    def __init__(self, key_id, key_secret):
        ...
        httpProfile.reqTimeout = (
            40000  # The streaming interface may take a longer time.
        )
...
class QwenClient(object):
    ...
    def qwen_api_call(
        ...
        """Use Qwen Chat API to perform text rewriting, parse <think>...</think> sections for
        reasoning content, and return (thinking, result)."""
        client = openai.OpenAI(
            base_url=self.base_url, api_key="None", timeout=600
        )
        messages = [
            {"role": "system", "content": system_prompt},
...
class QwenVLClient(object):
    def __init__(self, base_url=None, model_name=None):
        ...
        self.max_image_size = int(
            os.getenv("I2V_REWRITE_MAX_IMAGE_SIZE", "1024")
        )  # max image resolution fed to the model

    def _encode_image_to_base64(
        self, image_path: str, max_dimension: int
    ) -> str:
        """Load a local image, downscale it to max_dimension, and encode it
        ...
        try:
            with Image.open(image_path) as img:
                ...
                # Drop alpha channel; always convert to JPEG
    ...
    def qwen_api_call(
        ...
        for i in range(10):
            try:
                ...
                thinking = ""
                result = content or ""
                # Same as QwenClient: handle <think> reasoning-mode output
                if content and "</think>" in content:
                    head, tail = content.split("</think>", 1)
                    thinking = head.replace("<think>", "").strip()
                    result = tail.strip()
                ...
                last_err = e
                logger.error(
                    f"QwenVLClient request failed (attempt {i + 1}/10): {e}"
                )
                if i < 9:
                    time.sleep(2)
                else:
