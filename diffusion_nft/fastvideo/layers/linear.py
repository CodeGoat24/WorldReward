...
def adjust_scalar_to_fused_array(
    ...
    """For fused modules (QKV and MLP) we have an array of length N that holds 1 scale for each
    "logical" matrix.

    So the param is an array of length N. The loaded_weight corresponds to one of the shards on
    disk. Here, we slice the param based on the shard_id for loading.
    """
...
class MergedColumnParallelLinear(ColumnParallelLinear):
    ...
    def _load_fused_module_from_checkpoint(
        ...
        """Handle special case for models where MLP layers are already fused on disk. In this case,
        we have no shard id. This function determmines the shard id by splitting these layers and
        then calls the weight loader using the shard id.
...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    def _load_fused_module_from_checkpoint(
        ...
        """Handle special case for models where QKV layers are already fused on disk. In this case,
        we have no shard id. This function determmines the shard id by splitting these layers and
        then calls the weight loader using the shard id.
