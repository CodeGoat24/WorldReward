...
class BaseLayerWithLoRA(nn.Module):
    def __init__(
        ...
        if training_mode:
            ...
            # lora_B must be zero so that delta = x @ (B @ A) = 0 at init,
            # preserving the pretrained model's output before any training.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if (self.training_mode or not self.merged) and not self.disable_lora:
            ...
            # nn.Linear computes x @ W.T, so delta = x @ (B @ A).T = x @ A.T @ B.T
            lora_a = self.slice_lora_a_weights(lora_A.to(x, non_blocking=True))
            lora_b = self.slice_lora_b_weights(lora_B.to(x, non_blocking=True))
    ...
    def merge_lora_weights(self) -> None:
        ...
        if isinstance(self.base_layer.weight, DTensor):
            ...
            if isinstance(getattr(self.base_layer, "bias", None), DTensor):
                unsharded_base_layer.bias = nn.Parameter(
                    self.base_layer.bias.to(
                        get_local_torch_device(), non_blocking=True
                    )
                    .full_tensor()
                    .to(current_device)
                )
            ...
            self.base_layer.weight.data = data.to(
                current_device, non_blocking=True
            )
...
class ColumnParallelLinearWithLoRA(BaseLayerWithLoRA):
    ...
    def forward(self, input_: torch.Tensor) -> torch.Tensor:
        ...
        bias = (
            self.base_layer.bias if not self.base_layer.skip_bias_add else None
        )
...
class QKVParallelLinearWithLoRA(ColumnParallelLinearWithLoRA):
    ...
    def slice_lora_b_weights(
        ...
        return (
            B_q[q_start_idx:q_end_idx, :],
            B_kv[:, kv_start_idx:kv_end_idx, :],
        )
...
class LinearWithLoRA(BaseLayerWithLoRA):
    ...
    @property
    def weight(self):
        """Expose base_layer.weight so code like mlp[0].weight.dtype still works."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # No @torch.compile here: lora_A/lora_B are DTensors after FSDP set_trainable(),
        # and torch.compile's fake-tensor tracing fails on DTensor matmul shapes.
        lora_A = self.lora_A
        lora_B = self.lora_B
        if isinstance(self.lora_B, DTensor):
            lora_B = self.lora_B.to_local()
...
def get_lora_layer(
    ...
    supported_layer_types: dict[type[nn.Module], type[BaseLayerWithLoRA]] = {
        # the order matters – check parallel variants before nn.Linear
