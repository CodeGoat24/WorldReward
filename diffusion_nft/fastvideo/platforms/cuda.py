...
"""Code inside this file can safely assume cuda platform, e.g. importing pynvml.

However, it should not initialize cuda context.
"""
...
class CudaPlatformBase(Platform):
    ...
    def get_attn_backend_cls(
        ...
        logger.info(
            "Trying FASTVIDEO_ATTENTION_BACKEND=%s",
        ...
        if selected_backend == AttentionBackendEnum.SLIDING_TILE_ATTN:
            try:
                ...
                from fastvideo.layers.attention.backends.sliding_tile_attn import (  # noqa: F401
                    SlidingTileAttentionBackend,
                )

                logger.info("Using Sliding Tile Attention backend.")

                return "fastvideo.layers.attention.backends.sliding_tile_attn.SlidingTileAttentionBackend"
            ...
            try:
                ...
                from fastvideo.layers.attention.backends.sage_attn import (  # noqa: F401
                    SageAttentionBackend,
                )

                logger.info("Using Sage Attention backend.")

                return "fastvideo.layers.attention.backends.sage_attn.SageAttentionBackend"
            except ImportError as e:
                logger.info(e)
                logger.info(
            ...
            try:
                ...
                from fastvideo.layers.attention.backends.video_sparse_attn import (  # noqa: F401
                    VideoSparseAttentionBackend,
                )

                logger.info("Using Video Sparse Attention backend.")

                return "fastvideo.layers.attention.backends.video_sparse_attn.VideoSparseAttentionBackend"
            except ImportError as e:
                logger.error(
            ...
            return "fastvideo.layers.attention.backends.sdpa.SDPABackend"
        elif (
        ...
        if target_backend == AttentionBackendEnum.FLASH_ATTN:
            try:
                ...
                from fastvideo.layers.attention.backends.flash_attn import (  # noqa: F401
        ...
        return "fastvideo.layers.attention.backends.flash_attn.FlashAttentionBackend"
